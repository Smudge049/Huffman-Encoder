...
void insertMinHeap(struct MinHeap *minHeap, struct Node *node) {
  if (minHeap->size >= minHeap->capacity) {
    printf("Error: MinHeap capacity exceeded.\n");
    return;
  }
  ++minHeap->size;
...
void compressFile(const char *inputPath, const char *outputPath) {
  ...
  if (size == 1) {
    printf("\n[INFO] File contains only one unique character. Using simplified "
           "encoding.\n");
    // For single character, we still need a tree, but we'll create a dummy
    // parent This ensures the encoding/decoding logic works correctly
  }
  ...
  if (outSize > inSize) {
    printf("\n[WARNING] Compressed version is larger than original due to "
           "header overhead.");
...
void decompressFile(const char *inputPath, const char *outputPath) {
  ...
  for (int i = 0; i < uniqueCount; i++) {
    if (fread(&uniqueChars[i], 1, 1, in) != 1 ||
        fread(&uniqueFreqs[i], sizeof(uint32_t), 1, in) != 1) {
  ...
  while (decodedCount < totalChars) {
    ...
    if (bit == 0) {
      if (!curr->left) {
        printf("Error: Corrupted compressed data (invalid left branch).\n");
        break;
      }
      curr = curr->left;
    } else {
      if (!curr->right) {
        printf("Error: Corrupted compressed data (invalid right branch).\n");
        break;
      }
      curr = curr->right;
    }
...
int main(int argc, char *argv[]) {
  ...
  // This allows decompressing by clicking the .huf file if associated with this
  // exe
  if (argc >= 2) {
    char inputPath[512];
    ...
    if (is_directory(inputPath)) {
      ...
      printf("Path: %s\n", inputPath);
      printf(
          "This tool only processes individual files (txt, images, etc.).\n");
      printf("\nPress Enter to exit...");
      getchar();
      return 1;
    }
    ...
    // Check if it's a Huffman compressed file (.huf) - case insensitive
    int isHuf = (lastDot && (_stricmp(lastDot, ".huf") == 0));
    ...
    if (isHuf) {
      ...
      snprintf(outputPath, sizeof(outputPath), "%s.huf", inputPath);
